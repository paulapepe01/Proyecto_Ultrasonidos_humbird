...
SISTEMA_ACTUAL = SISTEMAS_OP[0]  # MacOS

NOMBRE_ALTAVOZ = "Humbird Speaker" 
VOLUMEN_SEGURO = 50
CARPETA_AUDIOS = os.path.join(os.path.dirname(__file__), "AUDIOS")
...
class AplicacionLaboratorio:
    """Interfaz Gráfica Nativa (Frontend) - A prueba de fallos visuales de macOS"""
    def __init__(self, root, controlador):
        self.root = root
        self.controlador = controlador
        self.reproduciendo = False

        self.root.title("Control Humbird")
        self.root.geometry("450x300")
        
        # IMPORTANTE: Usamos componentes nativos (tk), sin temas oscuros forzados
    ...
    def listar_archivos_wav(self):
        if not os.path.isdir(CARPETA_AUDIOS):
            return []
        return sorted(f for f in os.listdir(CARPETA_AUDIOS) if f.lower().endswith(".wav"))
    ...
    def crear_interfaz(self):
        ...
        # Separador manual básico
        tk.Frame(self.root, height=2, bd=1, relief=tk.SUNKEN).pack(fill=tk.X, padx=20, pady=10)

        # Selector de Archivos
        marco_archivo = tk.Frame(self.root)
        marco_archivo.pack(pady=10)
        
        tk.Label(marco_archivo, text="Archivo WAV:", font=("Arial", 12)).pack(side=tk.LEFT, padx=5)
        
        archivos_wav = self.listar_archivos_wav()
        if not archivos_wav:
            archivos_wav = ["(Sin archivos .wav)"]
            
        self.variable_seleccion = tk.StringVar(self.root)
        self.variable_seleccion.set(archivos_wav[0])
        
        self.combo_archivo = tk.OptionMenu(marco_archivo, self.variable_seleccion, *archivos_wav)
        ...
        # Botón Play
        self.btn_play = tk.Button(marco_controles, text="▶ PLAY", font=("Arial", 12, "bold"), width=10, command=self.lanzar_play, state=tk.DISABLED)
    ...
    def auto_inicializar(self):
        """Prepara el Bluetooth y el volumen en segundo plano al arrancar"""
        ...
        # Desbloqueamos el botón de Play
        self.btn_play.config(state=tk.NORMAL)

    def lanzar_play(self):
        archivo = self.variable_seleccion.get()
        if not archivo or archivo == "(Sin archivos .wav)":
        ...
        self.lbl_estado.config(text=f"Emitiendo: {archivo}", fg="blue")

        ruta = os.path.join(CARPETA_AUDIOS, archivo)
        
        # Lanzamos el audio en un hilo aparte para no congelar la ventana
        threading.Thread(target=self.hilo_reproduccion, args=(ruta,), daemon=True).start()

    def hilo_reproduccion(self, ruta):
        self.controlador.reproducir_tono(ruta)
        
        # Cuando el audio termina por sí solo, reseteamos los botones
        # Usamos self.root.after para evitar problemas de hilos con Tkinter
        self.root.after(0, self.reset_botones)

    def lanzar_stop(self):
        self.reproduciendo = False
        sd.stop()  # Aborta la señal directamente al sistema operativo
        self.lbl_estado.config(text="Emisión detenida.", fg="orange")
        self.reset_botones()
    ...
    def al_cerrar(self):
        """Si cierras la ventana mientras suena, corta el audio por seguridad."""
        sd.stop()
        self.root.destroy()
...
if __name__ == "__main__":
    ...
    ventana = tk.Tk()
    app = AplicacionLaboratorio(ventana, controlador)
    ventana.mainloop()